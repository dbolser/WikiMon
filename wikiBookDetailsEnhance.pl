...
if($dateTimeForComparision ne ""){
    $datetimeFormatNow = DateTime::Format::HTTP->
        parse_datetime($dateTimeForComparision);
}
...
sub getDetailsForEachPage {
    ...
    foreach ( @{ $allAuthors->{query}->{pages}->{$keyArr[0]}->{revisions}} )
    {
        ...
        if($datetimeFormatNow ne ""){
            $comparisionValue = DateTime->
                compare($datetimeFormat, $datetimeFormatNow);
        }
